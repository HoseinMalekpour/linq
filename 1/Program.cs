...
var x = File.ReadAllLines("C:\\Users\\Amin\\Desktop\\New folder (3)\\1\\1\\3.txt");
...
//  for Change format datatime mm/dd//yy ====>>>>  dd/mm//yy  for  convert string to  datetime 
DateTime modifidatetime(string x)
{
    ...
    j[0] = j[1];
    j[1] = n;
    string k = j[1] + "/" + j[0] + "/" + j[2];
    DateTime date = DateTime.Parse(k);
    return date.Date;
}


var dddd = persons.Select(x => new person2 (){Name =x.Name , brithdayfa = x.brithday  , Nasionalnumber =x.Nasionalnumber}).ToList();


var dddrfgfgd = persons.Select(x => new { Name = x.Name, brithdayfa = x.brithday, emad = x.ID }).ToList();
